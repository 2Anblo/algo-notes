...
class Solution {
    public long numberOfRightTriangles(int[][] grid) {
        ...
        int height = grid.length;       // 行数
        int width = grid[0].length;     // 列数

        int[] cntRow = new int[height];       // cntRow[i]：第 i 行有多少个 1
        int[] cntColumn = new int[width];     // cntColumn[j]：第 j 列有多少个 1

        // 统计每一行、每一列中 1 的数量
        for(int i = 0; i < height; i++){
            for(int j = 0; j < width; j++){
                if(grid[i][j] == 1){
                    cntRow[i]++;       // 当前行 1 的数量 +1
                    cntColumn[j]++;     // 当前列 1 的数量 +1
                }
            }
        }
        ...
        for(int i = 0; i < height; i++){
            for(int j = 0; j < width; j++){
                if(grid[i][j] == 1){
                    ...
                    // 当前点：
                    // 同一行可以选择 cntRow[i]-1 个点
                    // 同一列可以选择 cntColumn[j]-1 个点
                    // 两者组合形成一个直角三角形
                    ans += (long)(cntRow[i] - 1) * (cntColumn[j] - 1);
                }
            }
        }
...
class Solution {
    public int numberOfBoomerangs(int[][] points) {
        ...
        // key: 距离平方
        // value: 当前点到其他点距离为该值的点的数量
        Map<Integer, Integer> map = new HashMap<>();

        // 枚举回旋镖中间的点 i
        for (int[] i : points) {
            map.clear(); // 每换一个中心点，重新统计距离

            // 枚举其他点 j，计算 j 到 i 的距离
            for (int[] j : points) {
                ...
                // 欧氏距离平方
                // 不需要开根号，比较距离大小只需要比较平方
                int d2 = dx * dx + dy * dy;

                // 如果之前已经有 k 个点和 i 距离相同
                // 那么新增的点可以和这 k 个点组成 k * 2 个回旋镖
                // (j, i, oldPoint) 和 (oldPoint, i, j)
                ans += map.getOrDefault(d2, 0) * 2;
...
class Solution {
    public boolean find132pattern(int[] nums) {
        ...
        // 即 nums[2...n-1]
        for (int k = 2; k < n; k++) {
            rightAll.merge(nums[k], 1, Integer::sum);
        }

        // 枚举中间的 j，寻找 nums[i] < nums[k] < nums[j]
        for (int j = 1; j < n - 1; j++) {

            // 找右侧大于 leftMin 的最小数字
            // 即寻找可能的 "2"
            Integer rightCeiling = rightAll.ceilingKey(leftMin + 1);

            // 如果存在 nums[k] > nums[i]
            // 并且 nums[k] < nums[j]
            // 则形成 132 模式
            if (rightCeiling != null && rightCeiling < nums[j]) {
                return true;
            }
            ...
            // 移除当前 j+1 的数字
            // 因为下一轮 j 增加，右侧范围要缩小
            if (rightAll.merge(nums[j + 1], -1, Integer::sum) == 0) {
...
class NumArray:
    ...
    def __init__(self, nums: List[int]):
        # 创建前缀和数组，长度比原数组多1，方便处理边界情况
        # prefix[i] 表示 nums[0] 到 nums[i-1] 的元素和
        n = len(nums) + 1
        self.prefix = [0] * n
...
class NumArray:
    ...
    def __init__(self, nums: List[int]):
        # 前缀和数组，prefix[i] 表示 nums[0] 到 nums[i] 的元素和
        n = len(nums)
        self.prefix = [0] * n
    ...
    def sumRange(self, left: int, right: int) -> int:
        # 如果从第0个元素开始，直接返回 prefix[right]
        if left == 0:
            return self.prefix[right]
        else:
            # 区间和 = right位置前缀和 - left前一个位置前缀和
...
class Solution:
    def subarraySum(self, nums: List[int]) -> int:
        ...
        # 构造前缀和数组
        # prefixSum[i] 表示 nums[0] 到 nums[i-1] 的元素和
        prefixSum = [0] * (n + 1)
        ...
        for i in range(n):
            # 当前元素 nums[i] 需要计算它左边 nums[i] 个位置以内的子数组和
            # 起点不能小于 0
            start = max(0, i - nums[i])

            # 利用前缀和计算区间 nums[start] ~ nums[i] 的和
...
class Solution:
    def subarraySum(self, nums: List[int], k: int) -> int:
        ...
        # 记录出现过的前缀和次数
        preMap = {}
        preMap[prefixSum[0]] = 1  # 空数组前缀和为0出现一次

        ans = 0

        for i in range(n):
            # 如果存在一个之前的前缀和 = 当前前缀和 - k
            # 说明两者之间的子数组和为k
            need = prefixSum[i+1] - k

            if preMap.get(need) != None:
                ans += preMap[need]

            # 记录当前前缀和出现次数
            if preMap.get(prefixSum[i+1]) != None:
                preMap[prefixSum[i+1]] += 1
            else:
                preMap[prefixSum[i+1]] = 1
...
class Solution:
    def getSumAbsoluteDifferences(self, nums: List[int]) -> List[int]:
        ...
        for i in range(n):
            ...
            #
            # 合并化简：
            # preSum[n]-2*preSum[i]+nums[i]*(2*i-n)
            res[i] = preSum[n] - 2 * preSum[i] + nums[i] * (2*i-n)
...
class Solution:
    def buildArray(self, target: List[int], n: int) -> List[str]:
        ...
        for i in range(1, n + 1):
            stack.append(i)     # 当前数字入栈
            ans.append("Push")  # 记录 Push 操作

            l = len(stack)  # 当前栈的长度

            # 如果当前栈顶元素不是 target 中对应位置的元素
            if l <= len(target) and stack[l-1] != target[l-1]:
                ans.append("Pop")   # 记录 Pop 操作
                stack.pop(l-1)      # 弹出当前元素

            # 如果栈已经构造出了 target，结束
            elif l == len(target) and stack[l-1] == target[l-1]:
                break
